<template>
  <div class="p-6">
    <!-- Loading -->
    <div v-if="loading && !status" class="py-8 text-center font-mono text-sm text-text-muted">Loading...</div>
    ...
    <template v-else-if="status">
      ...
      <section>
        ...
        <div class="flex items-center gap-3">
          <button
            class="rounded bg-accent px-4 py-2 font-mono text-sm text-white transition hover:bg-accent-hover disabled:opacity-50"
            :disabled="reloading"
            @click="reloadConfig"
          >{{ reloading ? 'Reloading...' : 'Reload Config' }}</button>
          <span v-if="reloadMessage" class="text-sm" :class="reloadOk ? 'text-success' : 'text-error'">
            {{ reloadMessage }}
          </span>
        </div>
      </section>
...
import { adminReloadConfig } from "../../api.js";
...
async function reloadConfig() {
  ...
  reloadMessage.value = "";
  try {
    const res = await adminReloadConfig();
    if (res.ok) {
      reloadOk.value = true;
      reloadMessage.value = "Configuration reloaded successfully";
    } else {
      reloadOk.value = false;
      reloadMessage.value = res.error || "Reload failed";
    }
  } catch {
    reloadOk.value = false;
    reloadMessage.value = "Network error";
  } finally {
