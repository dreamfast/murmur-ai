...
func TestMockProvider_ReturnsResponses(t *testing.T) {
	...
	resp1, err := m.ChatCompletion(context.Background(), &ChatRequest{})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	resp2, err := m.ChatCompletion(context.Background(), &ChatRequest{})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestMockProvider_ReturnsErrors(t *testing.T) {
	...
	_, err := m.ChatCompletion(context.Background(), &ChatRequest{})
